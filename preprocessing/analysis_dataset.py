#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2019/11/27 18:04
# @Author : caius
# @Site : 
# @File : analysis_dataset.py
# @Software: PyCharm
'''
此文件用于分析原有数据集信息
...
def stati_image_size(image_dir, save_dir, big_w_dir):
    ...
    for w in sorted(w_count_dict.keys()):
        ...
        y[w] = w_count_dict[w]
    plot.plot_multi_line([x], [y], ['Width'], save_path='/media/data1/ocr_densenet/data/width.png', show=True)
    ...
    for r in sorted(r_count_dict.keys()):
        ...
        y[r] = r_count_dict[r]
    x = [8*(_+1) for _ in x]
    plot.plot_multi_line([x], [y], ['L/W'], save_path='/media/data1/ocr_densenet/data/ratio.png', show=True)
...
def stati_label_length(label_json, long_text_dir):
    ...
    for image, label in image_label_json.items():
        ...
        if l > 25:
            cmd = 'cp /media/data1/ocr_densenet/data/competition/train/{:s} {:s}'.format(image, long_text_dir)
            os.system(cmd)
    ...
    for l in sorted(l_count_dict.keys()):
        ...
        y[l] = l_count_dict[l]
    plot.plot_multi_line([x], [y], ['Word Number'], save_path='/media/data1/ocr_densenet/data/word_num.png', show=True)
...
def main():
    image_dir = '/media/data1/ocr_densenet/data/competition/train'
    save_dir = '/media/data1/ocr_densenet/data/files'
    big_w_dir = '/media/data1/ocr_densenet/data/big_w_dir'
    stati_image_size(image_dir, save_dir, big_w_dir)

    train_label_json = '/media/data1/ocr_densenet/data/train_alphabet.json'
    long_text_dir = '/media/data1/ocr_densenet/data/long_text_dir'

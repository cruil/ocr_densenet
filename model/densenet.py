...
def densenet121(pretrained=False, small=0,**kwargs):
    ...
    model = DenseNet(num_init_features=64, growth_rate=32, block_config=(6, 12, 24, 16), small=small,
                     **kwargs) # out feature 1000
    if pretrained:
        pretrained_dict = model_zoo.load_url(model_urls['densenet121'])  # get pretrained dict
        model_dict = model.state_dict()  # get model dict
        # 在合并前(update),需要去除/更改 pretrained_dict一些不需要的参数
...
def transfer_state_dict(pretrained_dict, model_dict):
    ...
    for k, v in pretrained_dict.items():
        if k in model_dict.keys():
            ...
            state_dict[k] = v
        elif str(k).replace('.1','1') == str(v) or str(k).replace('.2','2') == str(v):
            print("upadate key(s) in state_dict :{}".format(k))
            state_dict[k] = v
    return state_dict
...
def densenet161(pretrained=False, **kwargs):
    ...
    model = DenseNet(num_init_features=96, growth_rate=48, block_config=(6, 12, 36, 24),
                     **kwargs)
    if pretrained:
        pretrained_dict = torch.load(model_zoo.load_url(model_urls['densenet161']))  # get pretrained dict
        model_dict = model.state_dict()  # get model dict
        # 在合并前(update),需要去除/更改 pretrained_dict一些不需要的参数
        pretrained_dict = transfer_state_dict(pretrained_dict, model_dict)
        model_dict.update(pretrained_dict)  # 更新(合并)模型的参数

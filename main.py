#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time : 2019/11/27 18:47
# @Author : caius
# @Site : 
# @File : main.py
# @Software: PyCharm
# the code from https://github.com/jiangxiluning/ocr_densenet
...
class DenseNet121(nn.Module):
    ...
    def __init__(self, out_size):
        ...
        self.inplanes = 1024
        self.densenet121 = densenet.densenet121(pretrained=True, small=args.small)  # small = 1 pool = 0
...
def test(epoch, model, train_loader, phase='test'):
    ...
    mkdir('/media/data1/ocr_densenet/data/result')
    model.eval()
    f1score_list = []
    recall_list = []
    precision_list = []
    ...
    result_file = open('/media/data1/ocr_densenet/data/result/{:d}_{:s}_result.csv'.format(epoch, phase), 'w')
    ...
    names = []
    if phase != 'test':
        gt_file = open('/media/data1/ocr_densenet/data/result/{:d}_{:s}_gt.csv'.format(epoch, phase), 'w')
        gt_file.write('name,content\n')
        analysis_file = open('/media/data1/ocr_densenet/data/result/{:s}_{:s}_gt.csv'.format('analysis', phase), 'w')
        os.system('rm -r /media/data1/ocr_densenet/data/analysis/{:s}'.format(phase))
        labels_all = []
    probs_all = []
    for i, data in enumerate(tqdm(train_loader)):
        ...
        # print(feats.shape)
        #
        # print(probs.data.cpu().numpy().max(2).max(1).max(0).shape)
        probs_all.append(probs.data.cpu().numpy().max(2).max(1).max(0))  # min(1)返回该矩阵中该维度的最小值
        ...
        for patch_i, patch_pred in enumerate(preds):
            # print('patch_pred shape {}'.format(patch_pred.shape))
            for part_i, part_pred in enumerate(patch_pred):
                # print('part_pred shape {}'.format(part_pred.shape))
                new_set = set()
        ...
        for pred, label in zip(preds, labels):
            ...
            if phase == 'train_val':
                ...
                th = 0.6
            if f1score < th:
                save_dir = '/media/data1/ocr_densenet/data/analysis'
                if not os.path.exists(save_dir):
                    os.mkdir(save_dir)
                save_dir = os.path.join(save_dir, phase)
                if not os.path.exists(save_dir):
                    os.mkdir(save_dir)
                os.system('cp /media/data1/ocr_densenet/data/dataset/train/{:s} {:s}/{:d}_{:s}'.format(name, save_dir, 100000 + i, name))
    ...
    if phase != 'test':
        ...
        np.save('/media/data1/ocr_densenet/data/result/{:d}_{:s}_labels.npy'.format(epoch, phase), labels_all)
    result_file.close()
    os.system('cp /media/data1/ocr_densenet/data/result/{:d}_{:s}_result.csv /media/data1/ocr_densenet/data/result/{:s}_result.csv'.format(epoch, phase, phase))

    np.save('/media/data1/ocr_densenet/data/result/{:d}_{:s}_probs.npy'.format(epoch, phase), probs_all)
    with open('/media/data1/ocr_densenet/data/result/{:s}_names.json'.format(phase), 'w') as f:
...
def train_eval(epoch, model, train_loader, loss, optimizer, best_f1score=0, phase='train'):
    ...
    # for i, data in enumerate(train_loader):
        images, labels = [Variable(x.cuda(async=True)) for x in data[1:3]]  # 512 64 3 48    # label 48 1823
        weights = torch.from_numpy(get_weight(labels)).cuda(async=True) # 48 1823
        probs = model(images)
        ...
        if 'train' in phase:
            ...
            try:
                ...
                optimizer.step()
                loss_list.append([x.item() for x in loss_output])
            except:
                # pass
                traceback.print_exc()
...
def main():
    ...
    # 划分数据集
    test_filelist = sorted(glob(os.path.join(args.data_dir, 'test', '*')))  # 5664+
    trainval_filelist = sorted(glob(os.path.join(args.data_dir, 'train', '*'))) # 13318

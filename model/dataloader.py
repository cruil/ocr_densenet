...
class DataSet(Dataset):
    def __init__(self,
        ...
        self.rotate_range = rotate_range ## 图片旋转范围
        self.margin = margin
        self.image_names = image_names
        self.image_label_dict = image_label_dict
        self.transform = transform
        self.phase = phase  # train
        self.class_num = class_num  # 1823
        self.word_labels = {}
        self.image_size = image_size  # 512 64
        ...
        if self.phase != 'pretrain':
            for image_name in image_names:
                ...
                if image_name not in image_label_dict:
                    try:
                        image_label_dict[image_name] = image_label_dict[
                            image_name.replace('seg.', '').split('.png')[0] + '.png']
                    except:
                        image_label_dict[image_name] = ''
                word_label = np.zeros(class_num)
                label = image_label_dict[image_name]#   取出label
                for l in label.split():
                    word_label[int(l)] = 1
                self.word_labels[image_name] = word_label.astype(np.float32)  # 将worldlabel对应位置标1
    ...
    def __getitem__(self, index):
        ...
        if self.phase == 'pretrain':
            ...
            if os.path.exists(seg_name):
                ...
                image, word_label = random_crop_image(image_name, self.image_label_dict[image_name.split('/')[-1]],
                                                      self.image_size, self.class_num, self.phase, index, no_aug,
                                                      self.args)
            else:
                image, word_label = random_crop_image(image_name, self.image_label_dict[image_name.split('/')[-1]],
                                                      self.image_size, self.class_num, self.phase, index, no_aug,
...
def random_crop_image(image_name, text, image_size, class_num, phase, idx, no_aug, args):
    ...
    if args.hist:
        ...
        image = Image.open(image_name).convert('RGB')  # 用Image取出jpg
    x = np.array(image) # shape 256*48
    assert x.min() >= 0
    assert x.max() < 256
    ...
    if phase == 'train' and not no_aug:
        ...
        # 短边小于64， 直接填0
        h, w = image.size # 256 48 h,w
        if w < image_size[1] and h > 64:
            if get_random(idx + 3) < 0.3:
                image = np.array(image)
                start_index = (image_size[1] - w) // 2 # 浮点数报错
